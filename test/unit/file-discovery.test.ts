...
describe("findSrcFiles", () => {
  ...
  it("ignores node_modules and other common ignore dirs", () => {
  ...
  it("handles non-existent directories gracefully", () => {
    const files = findSrcFiles(join(tmpDir, "nonexistent"));
    expect(files).toEqual([]);
  });
  ...
  it("respects maxFiles limit", () => {
    ...
    const files = findSrcFiles(tmpDir, 10);
    expect(files.length).toBeLessThanOrEqual(10);
  });

  it("discovers files with various supported extensions", () => {
