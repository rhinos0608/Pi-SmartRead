...
- Replace `intent_read`, `repo_map`, or `search`. (Note: the helper tools `search_symbols`, `resolve_symbol`, and `find_callers` were consolidated into `search` and `find_symbol` for API simplicity.)
