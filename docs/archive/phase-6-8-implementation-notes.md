...
The call graph extraction now supports **Python, Go, and Rust** in addition to TypeScript, JavaScript, and TSX. This enables call-graph-based candidate expansion via `search` code enrichment and graph-aware retrieval phases in `intent_read`.
