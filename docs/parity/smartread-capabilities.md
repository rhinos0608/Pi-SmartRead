...
Core tools below are available through the Pi extension API (`src/index.ts`); listed experimental tools require configuration. Default MCP server surface is `inspect`, `grep`, and `skill`; other Pi tools require separate configuration or integration.
...
- `paths: Array<{ path: string; offset?: number; limit?: number }>` — Batch multi-file read (max 100)
...
- `contextLines: number` — Lines per match (default 2, max 20)
