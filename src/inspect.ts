...
async function executeQueryInspectDetails(
    ...
    if (depth === "deep") {
        // Deep path: retain grep + AST and add semantic, symbol, graph, and LSP evidence.
        ...
        if (parts.length === 0) {
            parts.push(`[No matches for "${query}". Try depth: "deep" to retain grep + AST and add semantic, symbol, graph, and LSP search.]`);
        }
        searchResult = {
