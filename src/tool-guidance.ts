...
const TOOL_GUIDE_LINES = [
  ...
  "- inspect { path }: directory → ranked repo map; file → structural facts (callers, parent, children, overrides, re-exports) + quality signals (complexity, public API, reuse, recency, tests, deprecation). Add inspect.navigation {operation, line, character, query, maxResults} for LSP goto/symbol/hover and inspect.diagnostics {waitMs, maxPerFile, maxFiles} for fresh LSP diagnostics. Inspect returns metadata evidence only — you must read a file before editing it.",
  "- grep { pattern }: primary code search — BM25 ranking + symbol matching + semantic fallback. Also accepts { queries: [...] } with 1-10 full search objects and grep.structural {language, skip, groupByFile} for ast-grep structural search. Grep returns search-match evidence only — you must read a file before editing it.",
