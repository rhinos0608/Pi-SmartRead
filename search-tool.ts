 ...
 * results into a single response. depth: "deep" adds fused semantic,
 * symbol, graph, and LSP channels with provenance.
 */
...
const SearchSchema = Type.Object({
  ...
  depth: Type.Optional(
    Type.Union([Type.Literal("quick"), Type.Literal("deep")], {
      description: "quick (default): grep + AST code search. deep: fused structural, semantic, symbol, graph, and LSP channels with provenance — use for broad or uncertain questions, or when quick returned nothing. Deep depth ignores matchMode/caseSensitive/contextLines.",
      default: "quick",
    }),
  ),
  scope: Type.Optional(
...
export default function createSearchTool(): ToolDefinition {
  return {
    ...
    label: "search",
    description:
        'Search repository text and AST-aware code definitions by exact term, regex, boolean query, or structural ast_pattern. Use for precise lookups, e.g. { query: "refreshToken" }, { query: "TODO|FIXME", matchMode: "regex" }, or { query: "class * extends Base", matchMode: "ast_pattern" }. Use depth: "deep" for broad cross-file search with fused semantic, symbol, graph, and LSP evidence and provenance. Prefer symbol when a symbol name is known and relationships matter, and read/read_files once target paths are known.',
